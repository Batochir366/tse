...
export default function CourseDetailPageClient() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setActiveLessonIdx(sorted.length > 0 ? 0 : null);
    } catch (err) {
  ...
  return (
    <div className="flex h-dvh min-h-0 flex-col overflow-hidden bg-surface-alt">
      {/* Header */}
      <header
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <h1 className="text-xs sm:text-xl font-bold tracking-tight text-foreground">TSE Academy</h1>
          </Link>
      ...
      <div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-y-contain">
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-6 pb-12">
          {loading && (
            ...
            <>
              ...
              <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
                <div className="flex-1 min-w-0 w-full space-y-6">
                  <motion.div
                    className="relative isolate aspect-video w-full max-w-full min-h-0 overflow-hidden rounded-2xl bg-zinc-900 shadow-sm"
                    style={{
                      boxShadow: `
                      ...
                      <>
                        <div className="absolute inset-0 overflow-hidden">
                          <iframe
                            key={heroDriveFileId}
                            src={heroDriveFileId}
                            title={activeLessonTitle ?? "Хичээлийн видео"}
                            className="absolute left-0 top-0 box-border h-full w-full max-h-full min-h-0 border-0"
                        ...
                        <button
                          ...
                          className="absolute z-10 flex h-10 w-10 touch-manipulation items-center justify-center rounded-full text-white transition-opacity hover:opacity-90 sm:h-9 sm:w-9"
                          style={{
